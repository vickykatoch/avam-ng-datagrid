...
export class CoolInfiniteGridComponent implements OnInit {
  ...
  @ContentChild(TemplateRef) private template: TemplateRef<Object>;
  ...
  @Input() public itemIterator: IIterator<any>;

  @Input() public itemWidth: any;

  @Input() public itemHeight: any;

  @Input() public itemSpace: any;
  ...
  public async ngOnInit(): Promise<any> {
    ...
    this.moveBottomBoundary =
      this.viewPortHeight * (1 - VIEW_PORT_MOVE_BOUNDARY_MULTIPLIER);
    ...
    function getScrollContainer() {
      ...
      while (currentNode) {
        if (
          ...
          currentNode.attributes[SCROLL_CONTAINER_ATTRIBUTE_NAME]
        ) {
          break;
        }
  ...
  private async handleCurrentScroll(scrollTop: number): Promise<any> {
    if (
      scrollTop > this.bottomViewPort.bottomScrollTop &&
      this.isLastViewPortRendered
    ) {
      return;
    }
    if (
      scrollTop < this.topViewPort.scrollTop ||
      scrollTop > this.bottomViewPort.bottomScrollTop
    ) {
  ...
  private calculateMoveBoundaries() {
    this.moveTopBoundary =
      this.topViewPort.scrollTop +
      this.viewPortHeight * (1 - VIEW_PORT_MOVE_BOUNDARY_MULTIPLIER);
  ...
  private get isLastViewPortRendered() {
    return (
      this.topViewPort.isLastViewPort ||
      this.middleViewPort.isLastViewPort ||
      this.bottomViewPort.isLastViewPort
    );
  }
  ...
  private async moveUpAsync(scrollTop: number): Promise<void> {
    ...
    newViewPort.height = this.calculateViewPortHeight(
      newViewPort.numberOfItems
    );
  ...
  private async moveDownAsync(scrollTop: number): Promise<void> {
    ...
    const fromIndex =
      this.bottomViewPort.itemsFromIndex + this.itemsPerViewPort;
  ...
  private async reRenderFromScrollAsync(scrollTop: number): Promise<void> {
    ...
    const viewPortItems = await this.getItems(
      fromIndex,
      3 * this.itemsPerViewPort
    );
    ...
    this.topViewPort.isLastViewPort =
      this.topViewPort.numberOfItems < this.itemsPerViewPort;
    this.topViewPort.height = this.calculateViewPortHeight(
      this.topViewPort.numberOfItems
    );
    ...
    this.middleViewPort.items = viewPortItems.slice(
      this.itemsPerViewPort,
      2 * this.itemsPerViewPort
    );
    this.middleViewPort.isLastViewPort =
      this.middleViewPort.numberOfItems < this.itemsPerViewPort;
    this.middleViewPort.height = this.calculateViewPortHeight(
      this.middleViewPort.numberOfItems
    );
    ...
    this.bottomViewPort.items = viewPortItems.slice(
      2 * this.itemsPerViewPort,
      3 * this.itemsPerViewPort
    );
    this.bottomViewPort.isLastViewPort =
      this.bottomViewPort.numberOfItems < this.itemsPerViewPort;
    this.bottomViewPort.height = this.calculateViewPortHeight(
      this.bottomViewPort.numberOfItems
    );
